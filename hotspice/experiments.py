...
class Sweep(ABC): # TODO: add a method to finish an unfinished sweep, by specifying an output directory to be completed
    ...
    def plot(self, summary_files, check=True, param_x=None, param_y=None, unit_x=None, unit_y=None, transform_x=None, transform_y=None,
             transform_z=None, name_x=None, name_y=None, names_z: list|str = None, n_xticks: int = None, n_yticks: int = None,
             ...
             fig_width: float = None, fig_height: float = None, panels_x: float = None, panels_y: float = None, pad: float = None,
             savefig_kwargs: dict = None, subplots_adjust_kwargs: dict = None):
        ...
        metrics_dict: dict[str, SweepMetricPlotparams] = experiment.get_plot_metrics()
        ...
        if is_2D:
            ...
            for i, (metric_key, params) in enumerate(metrics_dict.items()):
                ...
                if n_xticks is not None: ax.xaxis.set_major_locator(plt.MaxNLocator(n_xticks))
                if n_yticks is not None: ax.yaxis.set_major_locator(plt.MaxNLocator(n_yticks))
        ...
        if save:
            ...
            for ext in ('.pdf', '.png', '.svg'):
                ...
                save_plot(save_path, ext=ext, **savefig_kwargs) # Default dpi is 100, so save at higher res
        if plot:
            plt.show()
        plt.close()
...
class KernelQualityExperiment(Experiment):
    ...
    def get_plot_metrics() -> dict[SweepMetricPlotparams]:
        return {
            'K': SweepMetricPlotparams("Kernel-\nquality $K$", lambda data: data['K'], min_value=0), # TODO: can only add max_value=self.n_out if this is no longer a staticmethod
            'G': SweepMetricPlotparams("Generalisation-\ncapability $G$", lambda data: data['G'], min_value=0),
            'Q': SweepMetricPlotparams("Computing\ncapacity $C$", lambda data: np.maximum(0, data['K'] - data['G']), min_value=0)
        }
...
class TaskAgnosticExperiment(Experiment): # TODO: add a plot method to this class that plots the spatial metrics, and leave the total (non-local) metrics exposed for the Sweep.plot().
    ...
    def get_plot_metrics() -> dict[str:SweepMetricPlotparams]: # TODO: Better name to avoid confusion with self.plot(), add information about min and max values, name...
        return {
            'NL': SweepMetricPlotparams('Non-linearity', lambda data: data['NL'], min_value=0),
